...
public static class TimeFormat_TMP_Text
{
        ...
        string? prefixStr, string? suffixStr)
    {
        ...
        {
            TimeFormat.Set(pooledChars, eFormat, millSeconds, prefixStr, suffixStr);
        ...
        ReadOnlySpan<char> prefixChars, ReadOnlySpan<char> suffixChars)
    {
        using (PooledChars pooledChars = PooledChars.Get())
        {
            TimeFormat.Set(pooledChars, eFormat, millSeconds, prefixChars, suffixChars);
            self.SetText(pooledChars);
        }
    }
    public static void SetText(this TMP_Text self, TimeFormat.EFormat eFormat, long millSeconds)
    {
        using (PooledChars pooledChars = PooledChars.Get())
        {
            TimeFormat.Set(pooledChars, eFormat, millSeconds);
            self.SetText(pooledChars);
        }
    }
}
